...
const CryptoMetric = () => {
	...
	const percentageColor =
		cryptoSelected.modifiedPercentage > 0 ? colors.verde_claro : colors.rojo
	const percentageIcon =
		cryptoSelected.modifiedPercentage > 0 ? 'trending-up' : 'trending-down'
	...
	return (
		<ScrollView onLayout={onLayoutCryptoMetryc} className='bg-white'>
			<HeaderCryptoMetric />
			<View className='flex justify-between items-center p-6 bg-white rounded-t-3xl top-[-20px]'>
				<View className='flex flex-row justify-between p-3 rounded-3xl border border-solid border-gray-200 w-[50%] absolute bg-white top-[-40px]'>
					<TouchableOpacity
						...
						onPress={() => navigation.navigate('CryptoTransferencia')}
					>
						<SendToMobile color={colors.azul} size='35' />
						<Text
							className='text-azul pt-2 text-base'
			...
			<View className='bg-violeta_background p-4 rounded-xl my-4 mx-7'>
				<View className='mb-4 flex flex-row justify-center items-center'>
					<View className='flex flex-row items-center justify-center mr-5'>
						<Image
							source={{ uri: cryptoSelected.imgIcon }}
					...
					<View className='flex flex-row items-center'>
						...
						<Text
							className='text-3xl ml-4 font-semibold'
							style={{ color: percentageColor }}
						>
							{cryptoSelected?.modifiedPercentage}%
						</Text>
					</View>
				</View>
